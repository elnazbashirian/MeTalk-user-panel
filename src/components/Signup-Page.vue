    ...
    <div
      ...
      <div class="login-page-section  flex flex-col lg:w-1/2 w-full p-8">
        ...
        <div class="alreadey-a-member flex xl:flex-row flex-col mt-4 text-sm mb-4">
          ...
          <router-link class="text-blue-500 xl:mr-2 xl:mt-0 mt-2" to="/Login">
            وارد شوید
          </router-link>
        </div>
        <form class="flex flex-col" @submit.prevent="sendSignupAndValidate">
          <ValidationObserver ref="signup" class="w-full flex flex-col">
            <validation-provider v-slot="{ errors }" class="flex flex-col w-full"
              ...
              <input id="emailorphone"
                     v-model="username"
                     class="flex px-5 py-2 rounded-lg border border-solid border-gray-300" name="ایمیل یا شماره همراه"
            ...
            <validation-provider v-slot="{ errors }" class="flex flex-col w-full" rules="password|required">
              <input id=""
                     v-model="password"
                     class="flex w-full px-5 py-2 rounded-lg border border-solid border-gray-300" name="رمز عبور" placeholder="رمز عبور را وارد کنید" type="password">
              <span class="flex mt-2 text-xs text-red-500">{{ errors[0] }}</span>
            </validation-provider>
          </ValidationObserver>
          <!-- <button type="submit"
          ...
          <loading-btn :classes="['w-full', 'px-5', 'py-3', 'mt-12', 'bg-blue-500', 'hover:bg-blue-600', 'text-white', 'rounded-lg', 'font-bold']" :loading="loading.signup" :text="'ثبت نام'"
                       type="submit"/>
          ...
          <a class="flex flex-row mt-4 w-full px-5 py-3 bg-white text-gray-500 border border-solid border-gray-400 rounded-lg font-bold justify-center"
            ...
            <p class="flex" data-i18n-key="continuewithgoogle">با حساب گوگل ثبت نام کنید</p>
          </a>
        </form>
      </div>
      ...
      <div :class="{ 'show': isActive }"
        ...
        <div class="flex flex-col justify-center item-center content-center lg:mt-[70px] mt-[70px]">
          ...
          <form class="flex flex-col" @submit.prevent="sendVerifyCodeAndValidate">
            <ValidationObserver ref="verifycode" class="w-full flex flex-col">
              <validation-provider v-slot="{ errors }" class="flex flex-col w-full"
                                   rules="verifycode|required">
                <label class="mt-6 mb-4 text-sm" for="کد تایید">کد تایید</label>
                <input id=""
                       v-model="verifyCode"
                       class="flex px-5 py-2 rounded-lg border border-solid border-gray-300" name="کد تایید" placeholder="کد خود ارسال شده را وارد کنید" type="number">
                <span class="flex mt-2 text-xs text-red-500">{{ errors[0] }}</span>
              </validation-provider>
            </ValidationObserver>
            <span class="flex mx-auto mt-8">
                            <span :class="[allowSendVerifyCodeAgain ? 'text-blue-700' : 'text-gray-500   cursor-not-allowed']"
                                  class="cursor-pointer  ml-2"
                                  @click="sendSignupAndValidate">فرستادن دوباره کد تایید</span>
            ...
            <loading-btn :classes="['w-full', 'px-5', 'py-3', 'mt-12', 'bg-blue-500', 'hover:bg-blue-600', 'text-white', 'rounded-lg', 'font-bold']" :loading="loading.verifyCode" :text="'ثبت'"
                         type="submit"/>
          </form>
        </div>
      </div>
