  ...
  <div class="main-wrapper  bg-[#FAF9F8]">
    <div class="cover-hero">
      <div class="cover-image relative block">
        ...
        <div
            class="cover-image-wrapper absolute  right-0  bottom-0  top-0  h-full  w-full bg-[url('./assets/cover.jpeg')]  bg-no-repeat  bg-cover  bg-center"></div>
          ...
          <div class="flex-auto  flex  items-end">
            <div class="h-56 xl:block hidden  w-full  overflow-hidden p-4  pr-8">
              ...
              <span class="block  mt-3  text-white">{{
                  consultant?.email || 'ایمیلی برای این مشاور ثبت نشده است.'
                }}</span>
              <span class="block  mt-3  text-white">{{
                  consultant?.phone || 'شماره تلفنی برای این مشاور ثبت نشده است.'
                }}</span>
            </div>
          </div>
        </div>
        ...
        <img
            :src="consultant?.coverImage ? 'https://api.metalk.online/static/' + consultant?.coverImage : '/cover.png'"
            alt="profile card header"
            class="cover-backdrop z-10 top-0 right-0 bottom-0 absolute bg-gradient-to-t from-[rgba(0,0,0,.7)] to-[rgba(0,0,0,0)] flex w-full h-full">
      ...
      <div class="side-profile  z-30  relative xl:-top-56  xl:w-72 w-full xl:py-0 py-4  xl:mr-10  bg-white">
        <div class="profile-image xl:flex hidden relative">
          ...
          <img :src="consultant?.profileImage ? 'https://api.metalk.online/static/' + consultant?.profileImage : '/cover.png'"
               alt="profile card header"
               class="absolute top-0 left-0 bottom-0 bg-no-repeat bg-cover bg-center flex w-full h-full">
        ...
        <div :class="{showmore:checkStatus}" class="bio  p-3  px-4">
          ...
          <div class="social-medias">
            <ul class="social-meida-wrapper  flex  justify-center  py-4">
              <a class="flex  justify-center  items-center  w-10  h-10  rounded  transition-transform  hover:-translate-y-1 bg-[#3763d2]"
                 href="https://www.facebook.com/Odin-Design-Themes-and-Templates-1985202918413398/" target="_blank">
                ...
                {{ consultant?.socialMedias?.map(social => social.name).join(', ') }}
              </a>
            </ul>
          </div>
        </div>
        <button class="text-[11px] block px-[10px] py-[7px] rounded-md bg-slate-200 text-gray-700 mr-auto ml-4 mt-4"
                @click="changeData">{{ readStatus }}
        </button>
      </div>
...
<!--                        target="_blank"-->
            <a
                v-for="customer in consultant.customers"
                :key="customer.name + customer.url"
                :href="customer.url"
                target="_blank"
            ...
            >
              <img
                  :src="customer.image ? 'https://api.metalk.online/static/' + customer.image : '/img.png'"
                  style="width: 80px; height: 80px; border-radius: 100%; margin-left: 1em;"
              />
...
<!--          <span class="block  text-lg  mb-10  text-gray-700">آخرین مشاوره ها</span>-->
          ...
          <div v-else-if="consultations?.length !== 0 && !loadingConsultations">
            <div v-for="consultation, index in consultations" :key="consultation.id"
                 class="xl:h-16  mb-6  xl:rounded rounded-lg flex xl:flex-row flex-col  items-center xl:py-0 py-4  pl-8  pr-3 bg-white w-full">
              <div class="w-1/12">
                <div class="w-10 h-10  rounded  text-white  xl:flex hidden  items-center  justify-center bg-purple-600">
                  {{ index + 1 }}
                </div>
              </div>
              ...
              <div class="text-sm  overflow-hidden   xl:w-2/12 xl:my-0 my-2">
                ...
                <span class="text-gray-600 mr-1">{{
                    consultation.user?.firstname ? consultation.user?.firstname + ' ' + consultation.user?.lastname : 'کاربر میتاک'
                  }}</span>
              </div>
              <div class="text-sm  overflow-hidden   xl:w-2/12 xl:my-0 my-2">
          ...
          <div v-else>
            ...
            <div class="flex flex-col w-full h-full  items-center  justify-center">
              <img alt="" class="w-96  rounded-lg  border" src="/img/website-consulting-illustration.png">
              ...
              <button
                  class="bg-[#FAF9F8]  border  border-blue-600  text-blue-600  py-2  mt-8  hover:bg-blue-600  hover:text-white  transition-all  px-14  rounded">
                رزرو مشاوره
              </button>
            </div>
          </div>
